...
def build_generator(gan_input):
    with tf.compat.v1.variable_scope('generator'):
        ...
        # x = tf.compat.v1.layers.dropout(x, 0.1)
        x = tf.compat.v1.layers.dense(x, 28*28, activation='tanh')
...
def build_discriminator(disc_input, reuse=False):
    with tf.compat.v1.variable_scope('discriminator', reuse=reuse):
        x = tf.compat.v1.layers.dense(disc_input, 512, activation='relu')
...
def begin():
    ...
    disc_input_plc = tf.compat.v1.placeholder(tf.float32, [None, 784])
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for e in range(n_epoch):
            ...
            for b in range(n_batch):
                ...
                gen_noise_input = np.random.normal(size=[batch_sz, 100])
