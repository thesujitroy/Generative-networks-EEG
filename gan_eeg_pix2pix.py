...
def Discriminator():
  ...
  inp = tf.keras.layers.Input(shape=[40, 32, 3], name='input_image')
...
def train_step(input_image):
  with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
    ...
    gen_loss = generator_loss(disc_generated_output, gen_output, input_image) # changed traget to input_image
...
def generate_images(model, test_input):
  ...
  # (which we don't want)
  prediction = model(test_input[None,...], training=True)
  prediction = tf.compat.v1.keras.backend.eval(prediction[0])
...
def fit(train_data, epochs, batch_size):#, test_ds):
...
#    if epoch  % 10 == 0:
#        sample_id =  np.random.randint(4400)
#        xxy = train_data[sample_id]
#        generate_images(generator, xxy)
#    if epoch  == 149:
#        sample_id =  np.random.randint(4400)
#        xxy = train_data[sample_id]
#        generate_images(generator, xxy)
...
if __name__ == '__main__':
   ...
   #data, labels = run_sig_processing(data_src, labels_src, band_type=2)
    ...
    #gen_output = generator(xxx[0][tf.newaxis,...], training=False)

    fit(train_data, EPOCHS, BATCH_SIZE)
    generator.save('generator.h5')
    discriminator.save('generator.h5')
